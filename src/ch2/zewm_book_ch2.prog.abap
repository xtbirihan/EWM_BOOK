...
START-OF-SELECTION.
  ...
  lo_sp->query(
    EXPORTING
      ...
      options     = VALUE /scdl/s_sp_query_options( item_part_select = abap_true )
  ...
  IF lv_rejected EQ abap_true OR line_exists( lt_return_codes[ failed = abap_true ] ).
...
*           eo_message        =
      ...
      "Skip split items
      IF lv_cat = /scdl/if_dl_hierarchy_c=>sc_cat_ssp AND <srv_item_hierarchy>-parent_object IS NOT INITIAL.
    ...
    TRY.
        DATA(ls_status) = <srv_item>-status[  status_type = /scdl/if_dl_status_c=>sc_t_picking ].
    ...
    TRY.
        DATA(ls_addmeas) = <srv_item>-addmeas[ qty_role     = /scdl/if_dl_addmeas_c=>sc_qtyrole_pack
